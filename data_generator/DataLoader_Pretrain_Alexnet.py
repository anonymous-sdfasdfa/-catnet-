...
class CACD(data.Dataset):
    def __init__(self, split='train', transforms=None, label_transforms=None):
        ...
        data_root = "/home/wei/Documents/DATA/BMVC/ExperimentDataset/CACD/CACD2000"
        if split == "train":
...
class FFHQ(data.Dataset):
    def __init__(self, split='train', transforms=None, label_transforms=None):
        ...
        if split == "train":
            self.list_path=os.path.join(list_root,"ffhq_train.txt")
        else:
            self.list_path = os.path.join(list_root, "ffhq_test.txt")
        self.images_labels=[]#path
        self.transform=transforms
        self.label_transform=label_transforms
        with open(self.list_path) as fr:
