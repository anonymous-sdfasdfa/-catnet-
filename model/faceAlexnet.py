...
class AgeAlexNet_ffhq(nn.Module):
    def __init__(self,pretrainded=False,modelpath=None):
        super(AgeAlexNet_ffhq, self).__init__()
        assert pretrainded is False or modelpath is not None,"pretrain model need to be specified"
        self.features = nn.Sequential(
            Conv2d(3, 96, kernel_size=11, stride=4),
            nn.ReLU(inplace=True),
...
class AgeAlexNet_64(nn.Module):
    def __init__(self,pretrainded=False,modelpath=None):
        super(AgeAlexNet_64, self).__init__()
        assert pretrainded is False or modelpath is not None,"pretrain model need to be specified"
        self.features = nn.Sequential(
            Conv2d(3, 96, kernel_size=11, stride=4),
            nn.ReLU(inplace=True),
...
class AgeClassify:
    def __init__(self):
        #step 1:define model
        self.model=AgeAlexNet_64(pretrainded=False).cuda()
        # self.model = torch_model.alexnet(pretrained=True)
    ...
    def load_model(self,dir):
        self.model = AgeAlexNet_64(pretrainded=True, modelpath=dir).cuda()

class AgeClassify_ffhq:
    def __init__(self):
        #step 1:define model
        self.model=AgeAlexNet_ffhq(pretrainded=False).cuda()
        # self.model = torch_model.alexnet(pretrained=True)
        #step 2:define optimizer
        self.optim=torch.optim.Adam(self.model.parameters(),lr=1e-4,betas=(0.5, 0.999))
        #step 3:define loss
