...
for c = 1:nfold    
    ...
    name =['data/kfw1/' k_type '_' num2str(c) '_contrastive_IIATNet_kmm.mat'];
%     name =['data/kfw1/' k_type '_' num2str(c) '_contrastive_IIATNet_kmm_age.mat'];
    load(name);
    trainMask = fold ~= c;
    testMask = fold == c;    
...
%     saveW = ['weights/W_fs_' num2str(c) '.mat'];
%     save(saveW,'W');
    ts_Xa = ts_Xa * W;
    ts_Xb = ts_Xb * W;
...
avg_AUC(k)= AUC;
% save(['data/AUC/' kins(k,:) '_IIATNet_kmm_age.mat'],'fpr','tpr')
fprintf('%s AUC = %6.4f\n','fd',AUC)
figure
plot(fpr, tpr);
xlabel('False Positive Rate')
